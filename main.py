...
@app.route('/tour/<number>')
def open_tour_page(number):
    return render_template('tour.html',
                           tours=tours[int(number)],
                           departures=departures
                           )

@app.route('/departure/<town>')
def open_departure_page(town):
    tours_2 = {k: d for k, d in tours.items() if d["departure"] == town}
